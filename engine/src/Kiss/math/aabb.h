	...
	struct aabb {
		...
		inline aabb( v2* verts, const int count) {
			...
			for (int i = 0; i < count; ++i) {
				lmin = math::min(lmin, verts[i]);
				lmax = math::max(lmax, verts[i]);
			}
			min = lmin;
			max = lmax;
		}
		...
		static aabb fromCenter( v2 center,  v2 halfExtents)	{ return aabb(center - halfExtents, center + halfExtents); }
		static inline aabb clamp(aabb a, aabb b)						{ return aabb(math::clamp(a.min, b.min, b.max), math::clamp(a.max, b.min, b.max)); }
		...
		inline aabb merge( v2 p)   { return aabb(math::min(min, p), math::max(max, p)); }
		inline aabb merge( aabb b)  { return aabb(math::min(min, b.min), math::max(max, b.max)); }
		...
		inline v2 clamp( v2 p)	const	{ return math::clamp(p, min, max); }
		...
		inline f32 mergedArea( aabb b)const { return (math::max(max.x, b.max.x) - math::min(min.x, b.min.x))*(math::max(max.y, b.max.y) - math::min(min.y,b.min.y)); }
		...
		inline v2 wrap( v2 v) {
			const f32 dx	= math::abs(width());
			const f32 modx	= math::mod(v.x - min.x, dx);
			const f32 x		= (modx > 0.0f) ? modx : modx + dx;
			const f32 dy	= math::abs(height());
			const f32 mody	= math::mod(v.y - min.y, dy);
